...
const GAME_SETTINGS = {
  ...
  hitWallSoundUrl:
    "https://opengameart.org/sites/default/files/pingpongboard_0.ogg",
...
class TennisGame {
  constructor(canvas) {
    ...
    this._canvas.addEventListener("click", function (e) {
      ...
      if (
        that._startButtonRect.contains(mouseX, mouseY) &&
        !that._court.isMatchRunning
      )
...
class CpuPaddleController {
  ...
  update(deltaTime) {
    ...
    if (rng <= predictChance) {
      if (ballDeltaY > 0) this._paddle.moveUp(deltaTime);
      else this._paddle.moveDown(deltaTime);
    } else {
      rng = Math.random();

      if (rng < 0.2) {
        if (ballDeltaY > 0) this._paddle.moveDown(deltaTime);
        else this._paddle.moveUp(deltaTime);
      }
    }
  }
